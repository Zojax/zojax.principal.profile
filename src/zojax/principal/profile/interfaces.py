...
class IProfilesCategory(ICategory):
    ...
    photoAsAvatar = schema.Bool(
        title = _('Use photo as avatar'),
        description = _('Use photo as avatar. Users will need reupload photos.'),
        required = False,
        default = False)
...
class IProfileFields(interface.Interface):
    ...
    fields = schema.List(
        title = u'Fields',
        description = u'Determine the order and availability of fields.',
        value_type = schema.Choice(vocabulary='profile.fields'),
        default = [],
        required = False)
...
class IPersonalProfile(IPreferenceCategory):
    ...
    modified = schema.Datetime(
        title = _(u'Modified time'),
        description = _('Date when principal last modified its profile.'),
        required = False)

    def photoUrl(request):
        """Return url to personal photo."""
